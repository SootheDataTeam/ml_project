...
##### combined
select a1.neighborhood_bucket_id,a1.neighborhood_bucket,a1.city,a1.dayofweek,a1.day,a1.start_time,a1.end_time,a1.gender_requested,a1.Avg_cancellation,ifnull(a2.Avg_completed,0) as Avg_completed , a1.Avg_cancellation+ifnull(a2.Avg_completed,0) as Avg_potential,a1.predict_start_date,a1.predict_end_date
from
(select f.*
from
(select t.*,
...
when 'back_to_back' then session_gender_single 
 ...
 round(sum(case when appointment_type='couples' then 2 else 1 end)/(select cnt from (select dayname(local_date) as local_day,count(*)-1 as cnt
from
(
...
where m.local_day= dayname(ar.session_time)),2)as Avg_cancellation,sum(case when appointment_type='couples' then 2 else 1 end) as cnt
...
group by city,neighborhood_bucket_id,day,start_time, end_time,gender_requested
...
group by local_date) f
group by local_day)m
where m.local_day= dayname(ar.session_time)),2)as Avg_completed,sum(case when appointment_type='couples' then 2 else 1 end) as cnt
from appointment_requests ar        
left join (     
    select c.id, c.name
    from cities c
...
where nb.hidden <> 1
group by n.city_id,zip_code) nb on nb.zip_code=ar.customer_zip 
where   session_time>=last_day(now()) + interval 1 day -interval 3 month and session_time<= now() and ar.status in ('completed','filled')  
and neighborhood_bucket_id is not null and ar.id not in(select appointment_request_id from therapist_preferences where created_at>=last_day(now()) + interval 1 day -interval 3 month) 
group by city,neighborhood_bucket_id,day,start_time, end_time,gender_requested
having gender_requested is not null
order by dayofweek,start_time,end_time,gender_requested) t) f
...
) a2 on a1.city=a2.city and a1.neighborhood_bucket_id=a2.neighborhood_bucket_id and a1.day=a2.day and a1.start_time=a2.start_time and a1.end_time=a2.end_time and a1.gender_requested=a2.gender_requested

order by a1.Avg_cancellation desc
limit 50