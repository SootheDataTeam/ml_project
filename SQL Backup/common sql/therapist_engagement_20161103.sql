...
select *
from
(select u.id as user_id,ifnull(Engagment_score,0) as score ,ifnull(type,'D') as status
from

users u 
left join
(select id, 
case   round(Engagment_score,2) when 1.4 then 'A'
when 0.84 then 'B'
else 'C' end as type, Engagment_score 

from
(select id,  recency_score*0.28 as Engagment_score
from
(select r.id,
...
order  by Engagment_score desc) ss, 
...
order  by Engagment_score desc)a
