...
set @effort_score=0, @score_id='';

select *
from
(select u.id as user_id, ifnull(effort_score,0) as score, ifnull(type,'D') as status
from

users u 
left join
(select id,  
case when effort_score>=1.05 then 'AAA'
when effort_score>=0.95 then 'AA'
when effort_score>=0.6 then 'A'
when effort_score>=0.4 then 'B'
else 'C' end as type, effort_score

from
(select id, round(productivity_score*0.15+ effort_rate_score*0.1,2) as effort_score
from
(select r.id,
