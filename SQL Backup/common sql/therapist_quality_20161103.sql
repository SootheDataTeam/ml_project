...
select *
from
(select u.id as user_id, ifnull(quaility_score,0) as score ,ifnull(type,'D') as status
from

users u 
left join
(select id, 
case when quaility_score >=2.33 then 'AAA'
when quaility_score>=1.8 then 'AA'
when quaility_score>=1.6 then 'A'
when quaility_score>=1 then 'B'
else 'C' end as type,quaility_score

from
(select id,round(sp_score*0.28+client_rating*0.19,3) as quaility_score
from
(select r.id,
...
order  by quaility_score desc) ss, 
...
order  by quaility_score desc)a
