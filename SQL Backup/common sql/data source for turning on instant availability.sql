...
case when active_month>=20 then 5

 when active_month<20  and active_month>=12 then 4

when active_month<12  and active_month>=8 then 3

when active_month<8  and active_month>=4 then 2

when active_month<4  and active_month>=1 then 1
else 1 end as active_month_score


,instant_availability_qualified




from
(select u.id,u.instant_availability_qualified,round(ifnull(ifnull(cnt,0)/a.active_month,0),1) as monthly_sp,round(ifnull(f.completed_sessions/active_month,0),0) as productivity, recency,round(ifnull(client_rating,0),1) as client_rating,working_point,active_month
...
where u.kind='therapist' and id <>14655 and suspended<>1) r 