##### MT ranking
set @score_rank=0, @score_id='',@score_rank1=0, @score_id1='',@x1=now()-interval 1 month ;

select therapist_type,sum(case when a1.pt=ifnull(a2.pt,0) then 1 else 0 end) as keep_same,sum(case when a1.pt>ifnull(a2.pt,0) then 1 else 0 end) as downgrade,sum(case when a1.pt<ifnull(a2.pt,0) then 1 else 0 end) as upgrade
from
(

select therapist_id,
case when therapist_type='AAA' then 3 
when therapist_type='AA' then 2
when therapist_type='A' then 1 end as pt,therapist_type

from
(select u.id as therapist_id, concat(u.first_name," ",u.last_name) as name,mobile_number as phone,email,city,ifnull(quaility_score,0) as quaility ,ifnull(Engagment_score,0) as Engagment ,ifnull(effort_score,0) as effort,ifnull(Overall_score,0)as Overall_score ,ifnull(type,'D') as therapist_type
from
...
else 'C' end as type,quaility_score,Engagment_score,effort_score

from
(select id,sp_score*0.28 + productivity_score*0.15 + recency_score*0.28 + client_rating*0.19+ effort_rate_score*0.1 as Overall_score, @score_rank := IF(@score_id <> id, @score_rank + 1, 1) AS score_rank,sp_score*0.28+client_rating*0.19 as quaility_score, recency_score*0.28 as Engagment_score, productivity_score*0.15+effort_rate_score*0.1 as effort_score
from
(select r.id,
...
from appointments
where status in ('complete', 'reviewed')  and session_time<=@x1
...
where (a.status='complete' or a.status='reviewed') and a.session_time<=@x1) f

group by f.therapist_id) f on f.therapist_id=u.id



where u.kind='therapist' and id <>14655 and suspended<>1 and u.created_at<=@x1) r)s
...
(select count(*) as cnt
from users u
join (select distinct(therapist_id) from appointments where status in ('complete','reviewed') and session_time<=@x1) a on a.therapist_id=u.id
where u.kind='therapist' and   u.id <>14655 and suspended<>1  and u.created_at<=@x1
)u
 )u1 on u.id=u1.id
where u.kind='therapist' 
order  by Overall_score desc)a
where therapist_type in ('AAA','AA','A')) a1
...
case when therapist_type='AAA' then 3 
when therapist_type='AA' then 2
when therapist_type='A' then 1 end as pt
from
(select u.id as therapist_id, concat(u.first_name," ",u.last_name) as name,mobile_number as phone,email,city,ifnull(quaility_score,0) as quaility ,ifnull(Engagment_score,0) as Engagment ,ifnull(effort_score,0) as effort,ifnull(Overall_score,0)as Overall_score ,ifnull(type,'D') as therapist_type
from
...
left join
(select id, Overall_score,
case when score_rank1<=round(cnt*0.01,0) then 'AAA'
when score_rank1<=round(cnt*0.03,0) then 'AA'
when score_rank1<=round(cnt*0.1,0) then 'A'
when score_rank1<=round(cnt*0.2,0) then 'B'
else 'C' end as type,quaility_score,Engagment_score,effort_score

from
(select id,sp_score*0.28 + productivity_score*0.15 + recency_score*0.28 + client_rating*0.19+ effort_rate_score*0.1 as Overall_score, @score_rank1 := IF(@score_id1 <> id, @score_rank1 + 1, 1) AS score_rank1,sp_score*0.28+client_rating*0.19 as quaility_score, recency_score*0.28 as Engagment_score, productivity_score*0.15+effort_rate_score*0.1 as effort_score
from
(select r.id,
case when monthly_sp>=5.5 then 5
when monthly_sp<5.5 and monthly_sp>=4 then 4
...
where u.kind='therapist' 
order  by Overall_score desc)a
where therapist_type in ('AAA','AA','A'))a2 on a1.therapist_id=a2.therapist_id 

group by therapist_type
order by field(therapist_type,"AAA","AA","A")