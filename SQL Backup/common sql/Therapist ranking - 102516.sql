...
from
(select id,sp_score*0.28 + productivity_score*0.15 + recency_score*0.28 + client_rating*0.11+ effort_rate_score*0.1 as Overall_score, @score_rank := IF(@score_id <> id, @score_rank + 1, 1) AS score_rank
from
(select r.id,
...
from
(select u.id,round(ifnull(ifnull(cnt,0)/a.active_month,0),1) as monthly_sp,round(ifnull(f.completed_sessions/active_month,0),0) as productivity, recency,round(ifnull(client_rating,0),1) as client_rating,working_point,round(ifnull(co.cnt/active_month,0),0) 
...
left join (  
select therapist_id,count(*)  as cnt from cancel_therapist_off_appointment_occurrences
 group by therapist_id  
) co on co.therapist_id=u.id
