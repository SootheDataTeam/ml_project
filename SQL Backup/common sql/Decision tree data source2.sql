...
select a.user_id,case when sixtymin_session>0 then 1 else 0 end as sixtymin_session ,case when nintymin_session>0 then 1 else 0 end as nintymin_session,case when twohour_session>0 then 1 else 0 end as twohour_session,
...
group by user_id ) si on si.user_id=a.user_id
