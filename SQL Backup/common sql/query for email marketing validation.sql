...
when num_completed_requests>1 then 'C2 lapsed'
when num_completed_requests=1 and timestampdiff(day,last_booking_date,now())<60  then 'C1 last booking less than 60d' 
when num_completed_requests=1 and timestampdiff(day,last_booking_date,now())>=60  then 'C1 last booking more than 59d' 
when timestampdiff(day,u.created_at,now())<30 then 'NBLT30'
when timestampdiff(day,u.created_at,now())>=30 then 'NBGT29'
end as client_type, case when recency>2 * freq then 1 else 0 end as Risk,last_booking_date,date(created_at) as Account_Created_At
from
users u
left join
(SELECT
...
truncate((DATEDIFF(current_date(),min(DATE(ar.created_at)))/sum(case when ar.status ='completed' then 1 else 0 end))/7,0) as weekly_freq,max(convert_tz(session_time,'UTC',case timezone when 'Pacific Time (US & Canada)' then 'America/Los_Angeles' else timezone end)) as last_booking_date 
