...
### demand projection
select day_local,city,sum(case when appointment_type='couples' then 2 else 1 end) /2 as cnt
from
(select 
case timezone
...
order by (TIMESTAMPDIFF(SECOND, updated_at, session_time)) asc) )) and ar.session_time>=now()-interval 3 week ) a
where  date(date_local)>=date(now()-interval dayofweek(now()-interval 1 day) day-interval 2 week)
and date(date_local)<date(now()-interval dayofweek(now()-interval 1 day) day)
group by day_local,city

### productivity projection


select day_local,city,count(*)/count(distinct(therapist_id)) as productivity
from
(select 
case timezone
when 'Europe/London' then
...
join cities c on c.id=ar.city_id
where a.status in ('completed','reviewed') and ar.session_time>=now()-interval 3 week ) a
where  date(date_local)>=date(now()-interval dayofweek(now()-interval 1 day) day-interval 2 week)
and date(date_local)<date(now()-interval dayofweek(now()-interval 1 day) day)
group by day_local,city
...
select r.day_local,r.city,round(cnt/productivity,0) as min_supply,productivity,round(cnt,0) as projected_demand
from
(select day_local,city,sum(case when appointment_type='couples' then 2 else 1 end) /2 as cnt
from
(select 
...
join appointments a on a.appointment_request_id= ar.id
join cities c on c.id=ar.city_id
where a.status in ('completed','reviewed') and ar.session_time>=now()-interval 4 week ) a
where  date(date_local)>=date(now()-interval dayofweek(now()-interval 1 day) day-interval 3 week)
and date(date_local)<date(now()-interval dayofweek(now()-interval 1 day) day-interval 1 week)
group by day_local,city)p on r.day_local=p.day_local and r.city=p.city
order by r.day_local,r.city
...
select r.day_local,r.city,mt_engaged,filled, round(filled/mt_engaged,1) as actual_productivity,cnt as actual_demand
from
...
where  date(date_local)>=date(now()-interval dayofweek(now()-interval 1 day) day-interval 1 week)
and date(date_local)<date(now()-interval dayofweek(now()-interval 1 day) day)
group by day_local,city)r on l.day_local=r.day_local and l.city=r.city
