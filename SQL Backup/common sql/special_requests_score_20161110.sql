...
from (
  select u.id as user_id,
    ifnull(quality_score,0) as score,
    ifnull(type,'D') as status 
  from users u
  left join (
    select id,
      case when quality_score >=2.33 then 'AAA'
        when quality_score>=1.8 then 'AA'
        when quality_score>=1.6 then 'A'
        when quality_score>=1 then 'B'
        else 'C' end as type,quality_score
    from (
      select id,
        round(sp_score*0.28+client_rating*0.19,3) as quality_score
      from (
        select r.id,
