...
when onboard<=30 then 'B'  
else 'C' end as type,onboard



from
(select id,sp_score*0.28 + productivity_score*0.15 + recency_score*0.28 + client_rating*0.19+ effort_rate_score*0.1 as Overall_score, @score_rank := IF(@score_id <> id, @score_rank + 1, 1) AS score_rank,onboard
from
(select r.id,
...
else 0 end as effort_rate_score,onboard


from
(select u.id,round(ifnull(ifnull(cnt,0)/a.active_month,0),1) as monthly_sp,round(ifnull(f.completed_sessions/active_month,0),0) as productivity, recency,round(ifnull(client_rating,0),1) as client_rating,working_point,timestampdiff(day,created_at,now()) as onboard
