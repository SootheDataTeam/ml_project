...
SELECT
    ...
    max(CASE WHEN credit_amount>0 THEN 1 ELSE 0 END) AS credit_amount,
     max(CASE WHEN gift_amount>0 THEN 1 ELSE 0 END) AS gift_amount
	FROM
	(SELECT
	...
	session_type, appointment_type, session_length,ifnull(credit_amount,0) as credit_amount,ifnull(gift_amount,0) as gift_amount,
