...
select a.user_id, concat(u.first_name," ", u.last_name) as name, num_completed_requests as completed_appointments,weekly_freq, email,
...
when num_completed_requests>=1 then 'C' end as client_type, case when recency>2 * freq then 1 else 0 end as Risk,last_booking_date,date(created_at) as Account_Created_At
from
(SELECT
user_id,
...
where num_completed_requests>=1